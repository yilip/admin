...
$(function(){
	...
	$('#duty').datagrid({
		url: path + "/authority/role/getList",
		columns: columns,
		striped : true,
		pagination: true,
	...
	$('#closeBtn').click(function(){
		...
		submitHandler : function(form) {
			var url = path + '/authority/role/save';
			if ($('#uuid').val()) {
				url = path + '/authority/role/modify';
			}
...
function editWin(id) {
	...
	$('#form').show();
	$('#roleForm').form('clear');
	$('#roleForm').form('load', path + '/authority/role/getById?id=' + id);
	$("#add").show().dialog({
		title: "修改",
		height: 300,
		width: 500,
...
function removeRow(id) {
	...
	$.messager.confirm('提示', '删除角色会连同删除角色下的所有权限<br/>您确定要删除已选的角色吗？', function(r) {
		if (r) {
			$.ajax({
				type : "POST",
				url : path + '/authority/role/remove',
				data : {
					'id' : id
				},
				success : function() {
...
//            $("#dicForm").form('clear');
	...
	$('#resource').html('');
	$('#resource').tree({
		url : path + '/authority/role/getPermission?roleId='+roleId+'&pid=' + pid,
		checkbox:true,
		animate:true,
		...
		onBeforeExpand : function(node, param) {
			$('#resource').tree('options').url = path
					+ '/authority/role/getPermission?roleId='+roleId+'&pid=' + node.id;
		},
		//数据加载成功后，展开第一级树节点
...
//			}
	...
	$('#addPermission4RoleBtn').click(function(){
		...
		//添加权限
		$.ajax({
			url: path + '/authority/role/addPermission',
			type: "post",
			dataType: "json",
			data: {
				'roleId':roleId,
