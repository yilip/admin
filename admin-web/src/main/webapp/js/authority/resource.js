...
$(function(){
	...
	$('#resource').tree({
		url : path + '/authority/permission/getPermissionsByPid?pid=' + pid,
		animate:true,
		dnd:true,
...
//	       return;
	       $.ajax({
	   		url: path + '/authority/permission/orderBy',
	   		type: "post",
	   		async: false,
	   		dataType: "json",
	   		data: {
		...
		onBeforeExpand : function(node, param) {
			$('#resource').tree('options').url = path
					+ '/authority/permission/getPermissionsByPid?pid=' + node.id;
		},
	...
	$('#saveBtn').click(function(){
		...
		var url=path + '/authority/permission/save';
		if ($('#uuid').val()) {
			url=path + '/authority/permission/modify';
		}
...
function queryToTree(){
	$('#resource').tree('options').url = path+ '/authority/permission/getPermissionsByPid?pid=' + pid;
...
function removePermission(){
	$.messager.confirm('提示', '删除此节点会连同该节点下的所有权限<br/>您确定要删除已选的节点吗？', function(r) {
		if (r) {
			var node = $("#resource").tree("getSelected");
			$.ajax({
				url: path + '/authority/permission/remove',
				type: "post",
				dataType: "json",
				data: {
					'id':node.id
