$(function(){
	...
	var columns = [[
					...
					{field:'code',title:'值',width:300,sortable:true,formatter:function(val,row,index){
						if (row.displayName.match('password')) {
							return '******';
						}else{
							return val;
						}
	...
	$("#sysConfigForm").validate({
        ...
        submitHandler:function(form){
        	var url=path + '/sysconfig/save';
    		if ($('#sysConfigForm input[name="id"]').val()!='') {
    			url=path + '/sysconfig/modify';
    		}
...
function editWin(id) {
	$('#sysConfigForm').form('clear');
	$('#sysConfigForm').form('load', path + '/sysconfig/getById?id=' + id);
	$("#add").show().dialog({
		title: "修改",
		height: 300,
		width: 500,
...
function reloadCache() {
	$.ajax({
		type : "POST",
		url : path + '/sysconfig/reloadCache',
		data : {},
		success : function(data) {
			if (data) {
