...
<script type="text/javascript" src="<%=path%>/js/admin/adminList.js"></script>
	<form id="list_search">
		<div class="seach_div">
			...
			<div><label>部门:</label><input style="height: 25px;" id="dept" name="dept" class="easyui-combobox" url="<%=path %>/dictionary/getByType/dept" panelHeight="auto"/></div>
	...
	<div id="add">
		<form id="adminForm" method="post">
			...
			<div class="x-form-item">
				...
				<div class="x-form-element">
					<input id="dept" name="dept" class="easyui-combobox" url="<%=path %>/dictionary/getByType/dept" panelHeight="auto" validType="selectValueRequired['#adminForm #dept','部门']"  />
				</div>
			</div>
			<div class="x-form-item">
	...
	function operateData(val,row,index){
		...
		var returnStr='';
		///删除管理员
        returnStr+='<security:authorize access="hasPermission(null, \'security.operation.user_admin_delete\')"><a onClick=deleteAdmin("'+row.id+'","'+row.adminId+'")>删除</a></security:authorize>';
		//判断是否用【详细】的权限
